...
class Agent:
    def __init__(self,env,decay=False,epsilon=0.5):
        self.env = env
        self.Q = {}
        self.alpha = 0.1
        self.gamma = 0.9
    ...
    def get_new_action(self, state):
        if np.random.rand() < self.epsilon:
            return np.random.choice([0, 1])## random action retrieve a random action between 0 and 1
        else:
    ...
    ##Test the agent
    ##Return the number of wins, losses and draws
    def test(self, num_episodes):
        ...
        #pf.plot_winrate(winratebyepisode,decaying=False)
